...
const { form2Flow } = require('./flows/form2.flow.js');
...
const flowPrincipal = addKeyword(EVENTS.WELCOME) //Este es el flujo por donde pasan TODOS los mensajes
    .addAction(async (ctx, ctxFn) => {
        ...
        const keywords = ["hola", "buenas", "ola", "dias", "tardes", "noches"];
        const bodyText = ctx.body.toLowerCase();
        const containsKeyword = keywords.some(keyword => bodyText.includes(keyword));
        if (containsKeyword && ctx.body.length < 90) {
            return await ctxFn.gotoFlow(welcomeFlow) //Si, esta saludando
        } //No, no esta saludando

        //Cuarto, el usuario quiere abrir el menu?
        if (ctx.body.length > 8 || (!ctx.body.toLowerCase().includes("menu") && !ctx.body.toLowerCase().includes("menú") && ctx.body.length < 8)) {
...
const main = async () => {
    ...
    const adapterFlow = createFlow([flowPrincipal, voiceNoteFlow, menuFlow, formFlow, form2Flow, welcomeFlow, gptFlow, adminFlow, sendPdfFlow])
